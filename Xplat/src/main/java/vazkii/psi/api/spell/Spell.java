...
public final class Spell {
	...
	public static final StreamCodec<RegistryFriendlyByteBuf, Spell> STREAM_CODEC = StreamCodec.composite(
			...
			LazyStreamCodec.of(() -> SpellGrid.STREAM_CODEC), s -> s.grid,
			Spell::fromCodecData
	);
	...
	private List<ModInformation> getModInformationForCodec() {
		...
		PsiPlatform platform = PsiServices.load(PsiPlatform.class);
		for(var namespace : this.getPieceNamespaces()) {
			var optionalMod = platform.findMod(namespace);
	...
	public void writeToNBT(CompoundTag cmp) {
		...
		for(String namespace : getPieceNamespaces()) {
			...
			platform.findMod(namespace).ifPresent(mod -> nbt.putString(TAG_MOD_VERSION, mod.version()));
			modList.add(nbt);
		}
