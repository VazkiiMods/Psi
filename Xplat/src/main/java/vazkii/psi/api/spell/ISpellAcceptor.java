...
public interface ISpellAcceptor {
	...
	static boolean isAcceptor(ItemStack stack) {
		return !stack.isEmpty() && Objects.nonNull(PsiCapabilities.spellAcceptor(stack));
	}

	static boolean isContainer(ItemStack stack) {
		ISpellAcceptor capability = PsiCapabilities.spellAcceptor(stack);
		if(capability == null)
			return false;
	...
	static ISpellAcceptor acceptor(ItemStack stack) {
		return Objects.requireNonNull(PsiCapabilities.spellAcceptor(stack));
	}
