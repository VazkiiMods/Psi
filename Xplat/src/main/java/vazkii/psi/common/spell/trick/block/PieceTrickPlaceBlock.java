...
public class PieceTrickPlaceBlock extends PieceTrick {
	...
	public static void placeBlock(Player player, Level world, BlockPos pos, int slot, boolean particles, boolean conjure, Direction direction, Direction horizontalDirection) {
		...
		boolean canPlace = PsiWorldInteractions.canPlace(player, world, pos);
		if(state.isAir() || state.canBeReplaced() && canPlace) {

			if(conjure) {

				world.setBlockAndUpdate(pos, ModConjuredBlock.BLOCK.get().defaultBlockState());
			} else {
				...
				if(!stack.isEmpty() && stack.getItem() instanceof BlockItem) {
					...
					if(result != InteractionResult.FAIL) {
						...
						if(world.isClientSide()) {
							int count = player.isCreative() ? -1 : countRemaining(player, rem);
							PsiClientRuntime.showRemainingItems(rem, count);
						}
					}
				}
			}
	...
	private static int countRemaining(Player player, ItemStack displayStack) {
		int count = 0;
		for(int i = 0; i < player.getInventory().getContainerSize(); i++) {
			ItemStack stack = player.getInventory().getItem(i);
			if(!stack.isEmpty() && ItemStack.isSameItem(displayStack, stack)) {
