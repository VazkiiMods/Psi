...
public final class ContributorSpellCircleHandler {
	...
	public static void load(Properties props) {
		...
		for(String key : props.stringPropertyNames()) {
			...
			try {
				...
				LOGGER.error("Contributor {} has an invalid hexcode!", key);
			}
		}
		colormap = m;
	}
	...
	private static class ThreadContributorListLoader extends Thread {
		...
		public ThreadContributorListLoader() {
			...
			setDaemon(true);
			setUncaughtExceptionHandler((thread, err) -> LOGGER.error("Caught off-thread exception from {}: ", thread.getName(), err));
			start();
		}
		...
		public void run() {
			try {
				...
				LOGGER.info("Could not load contributors list. Either you're offline or github is down. Nothing to worry about, carry on~");
