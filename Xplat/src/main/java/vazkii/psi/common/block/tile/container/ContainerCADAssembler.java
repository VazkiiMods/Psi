...
public class ContainerCADAssembler extends AbstractContainerMenu {
	...
	public ContainerCADAssembler(int windowId, Inventory playerInventory, TileCADAssembler assembler) {
		super(ModCADAssemblerBlock.MENU.get(), windowId);
		...
		addSlot(new Slot(assemblerInv, EnumCADComponent.ASSEMBLY.ordinal() + 1, 120, 91));
		addSlot(new Slot(assemblerInv, EnumCADComponent.CORE.ordinal() + 1, 100, 91));
		addSlot(new Slot(assemblerInv, EnumCADComponent.SOCKET.ordinal() + 1, 140, 91));
		addSlot(new Slot(assemblerInv, EnumCADComponent.BATTERY.ordinal() + 1, 110, 111));
		addSlot(new Slot(assemblerInv, EnumCADComponent.DYE.ordinal() + 1, 130, 111));
		...
		for(int armorSlot = 0; armorSlot < 4; armorSlot++) {
			...
			addSlot(new Slot(playerInventory, playerSize - 2 - armorSlot,
				...
				public Pair<ResourceLocation, ResourceLocation> getNoItemIcon() {
					return Pair.of(InventoryMenu.BLOCK_ATLAS, emptyArmorSlotTexture(slot));
				}
			});
		}
	...
	private static ResourceLocation emptyArmorSlotTexture(EquipmentSlot slot) {
		String texture = switch(slot) {
		case HEAD -> "helmet";
		...
		return ResourceLocation.withDefaultNamespace("item/empty_armor_slot_" + texture);
	}

	public static ContainerCADAssembler fromBlockPos(int windowId, Inventory playerInventory, BlockPos pos) {
