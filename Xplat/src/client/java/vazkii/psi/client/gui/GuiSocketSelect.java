...
import vazkii.psi.mixin.client.AccessorKeyMapping;
...
public class GuiSocketSelect extends Screen {
		...
		if(segments != 0) {
			...
			for(int seg = 0; seg < segments; seg++) {
				...
				if(mouseInSector) {
					...
					if(!cadStack.isEmpty()) {
						int color = PsiClientRuntime.colorForCAD(cadStack);
		...
		for(int seg = 0; seg < segments; seg++) {
			...
			if(!stack.isEmpty()) {
				...
				if(seg == socketable.getSelectedSlot()) {
					int color = 0x00FF00;
					if(!cadStack.isEmpty()) {
						color = 0xFF0000 - PsiClientRuntime.colorForCAD(cadStack);
					}
					graphics.drawString(this.font, I18n.get("psimisc.selected"), (int) (xsp + width / 4.0f), (int) (ysp + font.lineHeight), color, true);
				}
	...
	public void tick() {
		...
		for(KeyMapping k : set) {
			KeyMapping.set(((AccessorKeyMapping) k).psi$getKey(), isKeyDown(k));
		}
	}

	public boolean isKeyDown(KeyMapping keybind) {
		InputConstants.Key key = ((AccessorKeyMapping) keybind).psi$getKey();
