...
public class TrickCraftingCategory implements IRecipeCategory<ITrickRecipe> {
	...
	public TrickCraftingCategory(IGuiHelper helper) {
		background = helper.createDrawable(PsiAPI.location("textures/gui/jei/trick.png"), 0, 0, 96, 41);
		icon = helper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(ModItems.psidust.get()));
		programmerHover = helper.createDrawable(PsiAPI.location("textures/gui/programmer.png"), 16, 184, 16, 16);
	}
	...
	public void getTooltip(@NotNull ITooltipBuilder tooltip, ITrickRecipe recipe, @NotNull IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
		if(recipe.getPiece() != null && onTrick(mouseX, mouseY)) {
			...
			SpellPieceRenderer.getTooltip(recipe.getPiece(), tooltips);
