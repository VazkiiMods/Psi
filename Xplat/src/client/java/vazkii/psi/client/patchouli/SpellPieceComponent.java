...
public class SpellPieceComponent implements ICustomComponent {
	...
	public void build(int componentX, int componentY, int pageNum) {
		this.x = componentX;
		this.y = componentY;
		this.piece = PsiAPI.SPELL_PIECE_REGISTRY.getOptional(ResourceLocation.parse(name.asString()))
				.map(type -> type.create(new Spell()))
	...
	public void render(GuiGraphics graphics, IComponentRenderContext context, float pticks, int mouseX, int mouseY) {
		...
		SpellPieceRenderer.draw(piece, graphics.pose(), buffer, 0xF000F0);
		buffer.endBatch();
