...
import vazkii.psi.mixin.client.AccessorAbstractTexture;
...
public class FXWisp extends TextureSheetParticle {
	...
	private static final ParticleRenderType NORMAL_RENDER = new PsiParticleRenderType() {
		private boolean previousBlur;
		private boolean previousMipmap;
		...
		public BufferBuilder begin(@NotNull Tesselator tessellator, @NotNull TextureManager textureManager) {
			...
			previousBlur = ((AccessorAbstractTexture) tex).psi$isBlur();
			previousMipmap = ((AccessorAbstractTexture) tex).psi$hasMipmap();
		...
		public void end() {
			...
			Minecraft.getInstance().getTextureManager().getTexture(TextureAtlas.LOCATION_PARTICLES)
					.setFilter(previousBlur, previousMipmap);
		}
