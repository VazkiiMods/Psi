...
public final class HUDHandler {
	...
	public static final LayeredDraw.Layer SOCKETABLE_EQUIPPED_NAME = (graphics, deltatracker) -> {
		if(!PsiEvents.post(new RenderPsiHudEvent(PsiHudElementType.SOCKETABLE_EQUIPPED_NAME)).isCanceled()) {
			renderSocketableEquippedName(graphics, deltatracker);
		}
	};
	public static final LayeredDraw.Layer HUD_ITEM = (graphics, deltatracker) -> {
		if(!PsiEvents.post(new RenderPsiHudEvent(PsiHudElementType.HUD_ITEM)).isCanceled()) {
	...
	public static final LayeredDraw.Layer PSI_BAR = (graphics, deltatracker) -> {
		if(!PsiEvents.post(new RenderPsiHudEvent(PsiHudElementType.PSI_BAR)).isCanceled()) {
	...
	public static final LayeredDraw.Layer REMAINING_ITEMS = (graphics, deltatracker) -> {
		if(!PsiEvents.post(new RenderPsiHudEvent(PsiHudElementType.REMAINING_ITEMS)).isCanceled()) {
			renderRemainingItems(graphics, deltatracker);
		}
	};

	public static void register(Registrar registrar) {
		registrar.register(PsiAPI.location("psi_bar"), PSI_BAR);
		registrar.register(PsiAPI.location("socketable_equipped_name"), SOCKETABLE_EQUIPPED_NAME);
		registrar.register(PsiAPI.location("remaining_items"), REMAINING_ITEMS);
		registrar.register(PsiAPI.location("hud_item"), HUD_ITEM);
	}

	public static void renderAll(GuiGraphics graphics, DeltaTracker deltaTracker) {
	...
	private static boolean showsBar(PlayerData data, ItemStack stack) {
		...
		IPsiBarDisplay display = PsiCapabilities.psiBarDisplay(stack);
		if(display != null)
	...
	public static void drawPsiBar(GuiGraphics graphics, DeltaTracker deltatracker) {
		...
		if(PsiConfig.client().contextSensitiveBar() && currPsi == totalPsi &&
		...
		boolean right = PsiConfig.client().psiBarOnRight();
	...
	private static void renderSocketableEquippedName(GuiGraphics graphics, DeltaTracker deltatracker) {
		Minecraft mc = Minecraft.getInstance();
		int toolHighlightTimer = ((AccessorGui) mc.gui).psi$getToolHighlightTimer();
		if(mc.player == null || toolHighlightTimer - 10 <= 0) {
			return;
		}
		...
		int ticks = toolHighlightTimer - 10;
	...
	private static void renderHUDItem(GuiGraphics graphics, DeltaTracker deltatracker) {
		...
		if(!stack.isEmpty() && stack.getItem() instanceof ItemVectorRuler ruler) {
			drawRulerHUD(graphics, ruler, stack);
		}
	...
	private static void drawRulerHUD(GuiGraphics graphics, ItemVectorRuler ruler, ItemStack stack) {
		String text = ruler.getVector(stack).toString();
		int width = Minecraft.getInstance().font.width(text);
		graphics.drawString(Minecraft.getInstance().font, text,
				(graphics.guiWidth() - width) / 2, graphics.guiHeight() / 2 + 10,
				0xFFFFFFFF, false);
	}
	...
	public interface Registrar {
		void register(ResourceLocation id, LayeredDraw.Layer layer);
	}
}
