...
public class RenderTileProgrammer extends TileEntityRenderer<TileProgrammer> {
    ...
    public void render(TileProgrammer te, float partialticks, MatrixStack ms, IRenderTypeBuffer buffers, int worldLight, int overlay) {
        if (te.isEnabled()) {
			...
			Material material = PsiAPI.getMiscMaterialFromAtlas(LibResources.GUI_PROGRAMMER);
			IVertexBuilder buffer = material.getVertexConsumer(buffers, GuiProgrammer::getBackgroundLayer);
			float x = -7, y = -7;
			float width = 174;
			float height = 184;
			float u = 0, v = 0;
			...
			buffer.vertex(mat, x, y + height, 0).color(1, 1, 1, a);
			buffer.texture(u * rescale, (v + height) * rescale).light(light).endVertex();
			buffer.vertex(mat, x + width, y + height, 0).color(1, 1, 1, a);
			buffer.texture((u + width) * rescale, (v + height) * rescale).light(light).endVertex();
			buffer.vertex(mat, x + width, y, 0).color(1, 1, 1, a);
			buffer.texture((u + width) * rescale, v * rescale).light(light).endVertex();
			buffer.vertex(mat, x, y, 0).color(1, 1, 1, a);
			buffer.texture(u * rescale, v * rescale).light(light).endVertex();
			ms.pop();
